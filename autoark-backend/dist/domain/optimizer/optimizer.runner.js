...
exports.OptimizerRunner = void 0;
const metrics_service_1 = require("../analytics/metrics.service");
...
const Campaign_1 = __importDefault(require("../../models/Campaign")); // 假设有 Campaign 模型获取 budget
/**
 * 优化运行器
 * 编排：获取数据 -> 应用策略 -> 执行动作
 */
class OptimizerRunner {
     ...
     * 运行 Campaign 优化
     */
    async runForCampaign(campaignId) {
        try {
            // 1. 获取聚合数据 (最近 7 天)
            ...
            if (!optState) {
                ...
                if (campaign) {
                    ...
                    logger_1.default.warn(`[Optimizer] Campaign ${campaignId} not found, skipping`);
                    return;
                }
            }
            // 构建上下文
            ...
            for (const policy of this.policies) {
                ...
                if (action.type !== 'NOOP') {
                    logger_1.default.info(`[Optimizer] Policy ${policy.name} triggered action ${action.type} for ${campaignId}`);
                    // 4. 执行动作
                    await execution_service_1.executionService.execute(campaignId, 'campaign', action, optState.accountId);
                    // 仅执行一个动作，避免冲突
                    break;
                }
            }
            ...
            logger_1.default.error(`[Optimizer] Failed to run for campaign ${campaignId}:`, error);
        }
    }
}
exports.OptimizerRunner = OptimizerRunner;
