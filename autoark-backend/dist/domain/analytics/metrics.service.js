...
class MetricsService {
    ...
    async getMetrics(filter) {
        try {
            ...
            if (filter.accountId)
                matchStage.accountId = filter.accountId;
            ...
            const results = await MetricsDaily_1.default.aggregate(pipeline);
            return results;
        }
        catch (error) {
            logger_1.default.error('[MetricsService] getMetrics failed:', error);
            throw error;
        }
    }
    /**
    ...
    async getEntitySummary(params) {
        ...
        // 获取每日趋势 (用于判断 trend)
        const dailyData = await MetricsDaily_1.default.find({
        ...
        if (dailyData.length >= 2) {
            ...
            if (avgRoas2 > avgRoas1 * 1.1)
                trend = 'up';
            else if (avgRoas2 < avgRoas1 * 0.9)
                trend = 'down';
        }
        return {
            entityId: params.entityId,
