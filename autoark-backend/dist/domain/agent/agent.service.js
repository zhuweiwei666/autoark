...
class AgentService {
    constructor() {
        ...
        if (LLM_API_KEY) {
            ...
            logger_1.default.info(`[AgentService] Initialized with model: ${LLM_MODEL}`);
        }
        else {
            logger_1.default.warn('[AgentService] LLM_API_KEY not configured');
        }
    }
    // ==================== Agent 配置管理 ====================
    ...
    async getAgents(filters = {}) {
        return agent_model_1.AgentConfig.find(filters).sort({ createdAt: -1 });
    }
    ...
    async deleteAgent(id) {
        return agent_model_1.AgentConfig.findByIdAndDelete(id);
    }
    // ==================== 智能报告生成 ====================
    /**
     * 生成每日报告
     */
    async generateDailyReport(date, accountId) {
        logger_1.default.info(`[AgentService] Generating daily report for ${date}, account: ${accountId || 'all'}`);
        // 获取数据
        const query = { date };
        if (accountId)
            query.accountId = accountId;
        // 聚合当日数据
        const metricsData = await MetricsDaily_1.default.aggregate([
        ...
        // 获取表现最好的广告系列
        const topCampaigns = await MetricsDaily_1.default.aggregate([
        ...
        // 识别需要关注的广告系列 (亏损)
        const losingCampaigns = await MetricsDaily_1.default.aggregate([
            { $match: { date, campaignId: { $exists: true, $ne: null }, ...(accountId ? { accountId } : {}) } },
            {
                $group: {
                    _id: '$campaignId',
        ...
        const needsAttention = losingCampaigns.map((c) => ({
            entityType: 'campaign',
            entityId: c._id,
            entityName: c.name,
        ...
        if (this.model) {
            try {
                ...
                logger_1.default.error('[AgentService] AI summary generation failed:', error.message);
            }
        }
        // 保存报告
        const report = await agent_model_1.DailyReport.findOneAndUpdate({ date, accountId: accountId || null }, {
            date,
            accountId,
            summary: {
                totalSpend: todayData.totalSpend,
                ...
                avgRoas,
                activeCampaigns: todayData.campaigns?.length || 0,
                profitableCampaigns: topCampaigns.filter((c) => c.roas > 1).length,
        ...
        logger_1.default.info(`[AgentService] Daily report generated: ${report._id}`);
        return report;
    }
    /**
     * AI 生成报告摘要
...
告警: ${data.alerts.length > 0 ? data.alerts.map((a) => a.message).join(', ') : '无'}

表现最好的广告系列:
${data.topPerformers.map((c) => `- ${c.name || c._id}: ROAS ${c.roas.toFixed(2)}, 消耗 $${c.spend.toFixed(2)}`).join('\n')}

需要关注:
${data.needsAttention.map((c) => `- ${c.entityName || c.entityId}: ${c.issue}`).join('\n') || '无'}
...
请返回 JSON 格式 (不要 Markdown):
{
  ...
  "recommendations": ["建议1", "建议2", "建议3"]
    ...
    async chat(userId, message, context) {
        ...
        // 获取或创建会话
        let conversation = await agent_model_1.AiConversation.findOne({
            userId,
            status: 'active',
            'context.accountId': context?.accountId,
        }).sort({ createdAt: -1 });
        if (!conversation) {
            conversation = new agent_model_1.AiConversation({
                userId,
                context,
                messages: [],
            });
...
## 历史对话
${conversation.messages.slice(-6).map((m) => `${m.role === 'user' ? '用户' : 'AI'}: ${m.content}`).join('\n')}
...
7. 如果需要更详细的数据，说明需要什么`;
        ...
        try {
            ...
            // 保存对话
            conversation.messages.push({ role: 'user', content: message, timestamp: new Date() }, { role: 'assistant', content: response, timestamp: new Date(), dataUsed: allData });
            await conversation.save();
            return response;
        }
        catch (error) {
            logger_1.default.error('[AgentService] Chat failed:', error.message);
     ...
     */
    async getAllAdvertisingData() {
        const today = (0, dayjs_1.default)().format('YYYY-MM-DD');
        const yesterday = (0, dayjs_1.default)().subtract(1, 'day').format('YYYY-MM-DD');
        const sevenDaysAgo = (0, dayjs_1.default)().subtract(7, 'day').format('YYYY-MM-DD');
        const fourteenDaysAgo = (0, dayjs_1.default)().subtract(14, 'day').format('YYYY-MM-DD');
        const thirtyDaysAgo = (0, dayjs_1.default)().subtract(30, 'day').format('YYYY-MM-DD');
        // 获取所有账户
        ...
        const last30DaysTrend = await MetricsDaily_1.default.aggregate([
            {
                $match: {
        ...
        // 2.1 最近7天趋势（用于对比）
        const last7DaysTrend = last30DaysTrend.filter((d) => d.date >= sevenDaysAgo);
        // 3. 分投手数据（从 campaign name 提取）
        const campaignsWithMetrics = await Campaign_1.default.aggregate([
            {
                $lookup: {
                    from: 'metricsdailies',
                    let: { cid: '$campaignId' },
        ...
        if (token) {
            for (const account of accounts.slice(0, 5)) { // 限制账户数量
                try {
                    const insights = await (0, insights_api_1.fetchInsights)(`act_${account.accountId}`, 'campaign', undefined, token, ['country'], { since: today, until: today });
        ...
        const allCampaignsToday = await MetricsDaily_1.default.aggregate([
            {
                $match: {
                    campaignId: { $exists: true, $ne: null },
                    date: today
        ...
        // 9. 最近7天广告系列表现对比
        const campaignTrends = await MetricsDaily_1.default.aggregate([
            {
                $match: {
                    campaignId: { $exists: true, $ne: null },
        ...
        const countryHistoricalTrend = await MetricsDaily_1.default.aggregate([
            {
                $match: {
                    campaignId: { $exists: true, $ne: null },
        ...
        const optimizerHistoricalTrend = await MetricsDaily_1.default.aggregate([
            {
                $match: {
                    campaignId: { $exists: true, $ne: null },
        ...
        const thisWeekStart = (0, dayjs_1.default)().startOf('week').format('YYYY-MM-DD');
        const thisWeekEnd = today;
        const lastWeekStart = (0, dayjs_1.default)().subtract(1, 'week').startOf('week').format('YYYY-MM-DD');
        const lastWeekEnd = (0, dayjs_1.default)().subtract(1, 'week').endOf('week').format('YYYY-MM-DD');
     ...
     * 获取素材级别数据供 AI 使用
     */
    async getMaterialMetricsForAI(startDate, endDate) {
        try {
            // 获取最近7天表现最好的素材
            const topMaterials = await MaterialMetrics_1.default.aggregate([
                {
                    $match: {
            ...
            // 获取表现最差的素材（高消耗低ROAS）
            const losingMaterials = await MaterialMetrics_1.default.aggregate([
                {
                    $match: {
                        date: { $gte: startDate, $lte: endDate },
                        spend: { $gt: 20 }
            ...
            // 素材类型统计
            const materialTypeStats = await MaterialMetrics_1.default.aggregate([
                {
                    $match: {
                        date: { $gte: startDate, $lte: endDate },
                        spend: { $gt: 0 }
            ...
            logger_1.default.error('[AgentService] Failed to get material metrics:', error);
            return {
                topMaterials: [],
                losingMaterials: [],
                materialTypeStats: [],
     ...
     * 获取账户健康度分析
     */
    async analyzeHealth(accountId) {
        const today = (0, dayjs_1.default)().format('YYYY-MM-DD');
        const yesterday = (0, dayjs_1.default)().subtract(1, 'day').format('YYYY-MM-DD');
        const sevenDaysAgo = (0, dayjs_1.default)().subtract(7, 'day').format('YYYY-MM-DD');
        const matchQuery = { campaignId: { $exists: true, $ne: null } };
        if (accountId)
            matchQuery.accountId = accountId;
        // 今日数据
        const todayMetrics = await MetricsDaily_1.default.aggregate([
        ...
        // 昨日数据
        const yesterdayMetrics = await MetricsDaily_1.default.aggregate([
        ...
        // 7天平均
        const weekMetrics = await MetricsDaily_1.default.aggregate([
    ...
    async runAgent(agentId) {
        ...
        if (effectiveMode === 'observe') {
            ...
            for (const op of operations) {
                op.status = 'pending';
                const saved = await new agent_model_1.AgentOperation(op).save();
                // TODO: 发送通知
            }
        }
     ...
     * 获取广告系列表现数据
     */
    async getCampaignPerformance(accountId, days) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        return MetricsDaily_1.default.aggregate([
            {
                $match: {
                    accountId,
                    campaignId: { $exists: true, $ne: null },
    ...
    needsApproval(operation, agent) {
        ...
        if (operation.action === 'budget_increase' || operation.action === 'budget_decrease') {
            ...
            if (changeAmount > agent.aiConfig.approvalThreshold)
                return true;
        }
        return false;
    }
    ...
    async executeOperation(operationId, agent) {
        ...
        try {
            ...
            await operation.save();
            logger_1.default.error(`[AgentService] Operation failed: ${operation._id}`, error);
...
请返回 JSON（不要代码块）：
{
  ...
  "optimizationTips": ["优化小贴士"]
     ...
     * 发送告警通知
     */
    async sendAlert(agent, alert) {
        if (!agent.alerts.enabled)
            return;
        for (const channel of agent.alerts.channels) {
            try {
                ...
                logger_1.default.error(`[AgentService] Failed to send alert via ${channel.type}:`, error);
            }
        }
    }
    ...
    async sendDingTalk(config, alert) {
        const axios = require('axios');
        const message = {
            msgtype: 'markdown',
            markdown: {
    ...
    // ==================== 获取待审批操作 ====================
    async getPendingOperations(filters = {}) {
        return agent_model_1.AgentOperation.find({ status: 'pending', ...filters })
            .populate('agentId')
            .sort({ createdAt: -1 });
    }
