...
exports.allocateAppForTask = allocateAppForTask;
exports.releaseAppLoad = releaseAppLoad;
exports.recordAppResult = recordAppResult;
exports.distributeTaskToApps = distributeTaskToApps;
exports.getSchedulerStatus = getSchedulerStatus;
exports.resetAllAppLoads = resetAllAppLoads;
exports.checkAndRecoverRateLimitedApps = checkAndRecoverRateLimitedApps;
 ...
 * 3. 历史成功率
 */
async function allocateAppForTask(taskId) {
    const apps = await getAvailableApps();
    if (apps.length === 0) {
        logger_1.default.warn(`[TaskScheduler] No available apps for task ${taskId}`);
        return null;
    }
    // 选择负载最低的 App
    const selectedApp = apps[0];
    // 更新 App 负载
    await FacebookApp_1.default.updateOne({ appId: selectedApp.appId }, {
        $inc: { 'currentLoad.activeTasks': 1 },
        $set: { 'stats.lastUsedAt': new Date() }
    });
    logger_1.default.info(`[TaskScheduler] Allocated app ${selectedApp.appName} (${selectedApp.appId}) for task ${taskId}, current load: ${selectedApp.currentLoad + 1}/${selectedApp.maxConcurrent}`);
    return selectedApp.appId;
}
/**
 * 释放 App 负载
 */
async function releaseAppLoad(appId) {
    await FacebookApp_1.default.updateOne({ appId }, {
        $inc: { 'currentLoad.activeTasks': -1 },
    });
    // 确保负载不会变成负数
    await FacebookApp_1.default.updateOne({ appId, 'currentLoad.activeTasks': { $lt: 0 } }, { $set: { 'currentLoad.activeTasks': 0 } });
}
/**
 * 记录 App 请求结果
 */
async function recordAppResult(appId, success, error) {
    const update = {
        $inc: {
    ...
    if (!success && error) {
        ...
        if (error.toLowerCase().includes('rate limit') || error.toLowerCase().includes('too many')) {
            ...
            logger_1.default.warn(`[TaskScheduler] App ${appId} hit rate limit, will reset in 1 hour`);
        }
    }
 ...
 * @returns 分配结果
 */
async function distributeTaskToApps(taskId, accountIds) {
    const apps = await getAvailableApps();
    if (apps.length === 0) {
        logger_1.default.warn(`[TaskScheduler] No available apps, all accounts unallocated`);
    ...
    for (const app of apps) {
        ...
        if (availableSlots <= 0)
            continue;
        // 按比例分配任务
        const allocateCount = Math.min(availableSlots, Math.ceil((accountIds.length - accountIndex) * (availableSlots / totalAvailableSlots)));
        if (allocateCount > 0) {
    ...
    for (const dist of distributions) {
        await FacebookApp_1.default.updateOne({ appId: dist.appId }, {
            $inc: { 'currentLoad.activeTasks': dist.accountIds.length },
            $set: { 'stats.lastUsedAt': new Date() }
        });
    }
    logger_1.default.info(`[TaskScheduler] Task ${taskId} distributed: ${distributions.length} apps, ${unallocated.length} unallocated`);
...
async function getSchedulerStatus() {
    const apps = await FacebookApp_1.default.find().lean();
 ...
 */
async function resetAllAppLoads() {
    await FacebookApp_1.default.updateMany({}, {
        $set: {
    ...
    logger_1.default.info('[TaskScheduler] All app loads reset');
}
/**
 * 检查并恢复限流的 App
 */
async function checkAndRecoverRateLimitedApps() {
    const now = new Date();
    await FacebookApp_1.default.updateMany({
        status: 'rate_limited',
