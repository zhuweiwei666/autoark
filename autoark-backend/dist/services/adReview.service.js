...
exports.REJECTION_REASON_MAP = exports.REVIEW_STATUS_MAP = void 0;
exports.fetchAdReviewStatus = fetchAdReviewStatus;
exports.updateTaskAdsReviewStatus = updateTaskAdsReviewStatus;
exports.getTaskReviewDetails = getTaskReviewDetails;
exports.getReviewOverview = getReviewOverview;
exports.refreshAllReviewStatus = refreshAllReviewStatus;
 ...
 */
async function fetchAdReviewStatus(adIds, token) {
    ...
    try {
        ...
        for (let i = 0; i < adIds.length; i += batchSize) {
            ...
            for (const adId of batch) {
                try {
                    const response = await facebookClient_1.facebookClient.get(`/${adId}`, {
                        access_token: token,
                    ...
                    logger_1.default.warn(`[AdReview] Failed to fetch status for ad ${adId}:`, err.message);
        ...
        logger_1.default.error('[AdReview] Batch fetch failed:', error);
    }
    return results;
}
/**
 * 批量获取 Campaign 名称
 */
async function fetchCampaignNames(campaignIds, token) {
    const results = new Map();
    for (const campaignId of campaignIds) {
        try {
            const response = await facebookClient_1.facebookClient.get(`/${campaignId}`, {
                access_token: token,
            ...
            logger_1.default.warn(`[AdReview] Failed to fetch campaign ${campaignId}:`, err.message);
        }
    }
    return results;
}
 ...
 * 批量获取 AdSet 名称
 */
async function fetchAdSetNames(adsetIds, token) {
    const results = new Map();
    for (const adsetId of adsetIds) {
        try {
            const response = await facebookClient_1.facebookClient.get(`/${adsetId}`, {
                access_token: token,
                fields: 'id,name',
            });
            ...
            logger_1.default.warn(`[AdReview] Failed to fetch adset ${adsetId}:`, err.message);
        }
    }
    return results;
}
 ...
 * 解析审核反馈，提取被拒原因
 */
function parseReviewFeedback(feedback) {
    if (!feedback)
        return null;
    const parsed = {
 ...
 * 更新任务中所有广告的审核状态
 */
async function updateTaskAdsReviewStatus(taskId) {
    const result = {
        total: 0,
        updated: 0,
        pending: 0,
    ...
    try {
        ...
        // 获取有效的 token
        const activeToken = await FbToken_1.default.findOne({ status: 'active' });
        if (!activeToken) {
        ...
        for (const [adId, data] of statusMap) {
            ...
            // 更新或创建广告记录
            await Ad_1.default.findOneAndUpdate({ adId }, {
                $set: {
        ...
        logger_1.default.info(`[AdReview] Task ${taskId} review status updated: ${result.approved} approved, ${result.pending} pending, ${result.rejected} rejected`);
    }
    catch (error) {
        logger_1.default.error('[AdReview] Update task ads review status failed:', error);
 ...
 * 获取任务的广告审核状态详情
 */
async function getTaskReviewDetails(taskId) {
    const ads = await Ad_1.default.find({ taskId }).lean();
    const result = {
        summary: {
            total: ads.length,
            pending: 0,
    ...
    for (const ad of ads) {
        const statusInfo = exports.REVIEW_STATUS_MAP[ad.effectiveStatus || ''] || {
 ...
 */
async function getReviewOverview() {
    // 获取所有 AutoArk 发布的广告
    const ads = await Ad_1.default.find({ taskId: { $exists: true } }).lean();
 ...
 */
async function refreshAllReviewStatus() {
    const result = {
        total: 0,
        updated: 0,
        errors: [],
    ...
    try {
        // 获取所有 AutoArk 发布的广告
        const ads = await Ad_1.default.find({ taskId: { $exists: true } }).limit(1000);
        ...
        for (const [adId, data] of statusMap) {
            ...
            await Ad_1.default.findOneAndUpdate({ adId }, { $set: updateData });
            result.updated++;
        }
        // 批量获取 Campaign 名称并更新
        if (campaignIdsToFetch.size > 0) {
            logger_1.default.info(`[AdReview] Fetching names for ${campaignIdsToFetch.size} campaigns...`);
            ...
            for (const [campaignId, campaignName] of campaignNames) {
                await Ad_1.default.updateMany({ campaignId }, { $set: { campaignName } });
            }
            logger_1.default.info(`[AdReview] Updated ${campaignNames.size} campaign names`);
        }
        // 批量获取 AdSet 名称并更新
        if (adsetIdsToFetch.size > 0) {
            logger_1.default.info(`[AdReview] Fetching names for ${adsetIdsToFetch.size} adsets...`);
            ...
            for (const [adsetId, adsetName] of adsetNames) {
                await Ad_1.default.updateMany({ adsetId }, { $set: { adsetName } });
            }
            logger_1.default.info(`[AdReview] Updated ${adsetNames.size} adset names`);
        }
        logger_1.default.info(`[AdReview] Refresh all completed: ${result.total} total, ${result.updated} updated`);
    }
    catch (error) {
        logger_1.default.error('[AdReview] Refresh all failed:', error);
        result.errors.push(error.message);
    }
    return result;
}
 ...
 * 用于定时任务
 */
async function checkPendingAdsReview() {
    const result = {
        checked: 0,
        updated: 0,
        errors: [],
    ...
    try {
        ...
        const pendingAds = await Ad_1.default.find({
            $or: [
        ...
        if (pendingAds.length === 0) {
            logger_1.default.info('[AdReview] No pending ads to check');
            return result;
        }
        ...
        logger_1.default.info(`[AdReview] Batch check completed: ${result.checked} checked, ${result.updated} updated`);
    }
    catch (error) {
        logger_1.default.error('[AdReview] Batch check failed:', error);
        result.errors.push(error.message);
    }
    return result;
}
