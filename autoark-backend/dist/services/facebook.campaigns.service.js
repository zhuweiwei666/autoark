                ...
                for (const camp of campaigns) {
                    ...
                    if (insights && insights.length > 0) {
                        for (const insight of insights) {
                            ...
                            // 新的队列系统会使用 BulkWrite
                            await MetricsDaily_1.default.findOneAndUpdate({ campaignId: metricsData.campaignId, date: metricsData.date, country: country || null }, {
                                $set: metricsData,
...
const getCampaigns = async (filters = {}, pagination) => {
    ...
    const campaignsWithMetrics = campaigns.map(campaign => {
        ...
        if ((purchase_value === undefined || purchase_value === null) && actionValues && actionValues.length > 0) {
            ...
            const purchaseAction = actionValues.find((a) => a.action_type === 'purchase' || a.action_type === 'mobile_app_purchase');
