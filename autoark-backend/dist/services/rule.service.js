...
function getDateRange(timeRange) {
    ...
    switch (timeRange) {
        ...
        case 'last_3_days':
            return { start: (0, dayjs_1.default)().subtract(3, 'day').format('YYYY-MM-DD'), end: today };
        case 'last_7_days':
            return { start: (0, dayjs_1.default)().subtract(7, 'day').format('YYYY-MM-DD'), end: today };
        case 'lifetime':
 ...
 * 获取实体的指标数据
 */
async function getEntityMetrics(entityLevel, entityId, timeRange) {
 ...
 */
async function getAvailableToken(accountId) {
...
class RuleService {
    ...
    async getRules(filters) {
        ...
        return AutoRule_1.AutoRule.find(query).sort({ createdAt: -1 });
    }
    /**
     * 获取单个规则
     */
    async getRuleById(id) {
        return AutoRule_1.AutoRule.findById(id);
    }
    /**
     * 创建规则
     */
    async createRule(data) {
        const rule = new AutoRule_1.AutoRule(data);
        await rule.save();
        logger_1.default.info(`[RuleService] Created rule: ${rule.name} (${rule._id})`);
        return rule;
    }
    /**
     * 更新规则
    ...
    async updateRule(id, data) {
        const rule = await AutoRule_1.AutoRule.findByIdAndUpdate(id, data, { new: true });
        if (rule) {
            logger_1.default.info(`[RuleService] Updated rule: ${rule.name}`);
        }
        return rule;
    }
    /**
     ...
     */
    async deleteRule(id) {
        const result = await AutoRule_1.AutoRule.findByIdAndDelete(id);
        return !!result;
    }
    /**
     * 执行单个规则
    ...
    async executeRule(ruleId) {
        const rule = await AutoRule_1.AutoRule.findById(ruleId);
        if (!rule) {
        ...
        logger_1.default.info(`[RuleService] Executing rule: ${rule.name}`);
        const execution = {
            executedAt: new Date(),
            entitiesChecked: 0,
            entitiesAffected: 0,
        ...
        try {
            ...
            logger_1.default.info(`[RuleService] Checking ${entities.length} entities for rule: ${rule.name}`);
            // 逐个检查和执行
            for (const entity of entities) {
                // 检查执行限制
                if (rule.limits.maxEntitiesPerExecution &&
                    execution.entitiesAffected >= rule.limits.maxEntitiesPerExecution) {
                    logger_1.default.info(`[RuleService] Max entities limit reached: ${rule.limits.maxEntitiesPerExecution}`);
                    break;
                }
                // 获取指标并检查条件
                const metrics = await getEntityMetrics(rule.entityLevel, entity.id, rule.conditions[0]?.timeRange || 'last_3_days');
                // 检查所有条件（AND 逻辑）
            ...
            await rule.save();
            logger_1.default.info(`[RuleService] Rule ${rule.name} executed: ${execution.entitiesAffected}/${execution.entitiesChecked} affected`);
        }
        catch (error) {
            logger_1.default.error(`[RuleService] Rule execution failed: ${error.message}`);
    ...
    async getEntitiesToCheck(rule) {
        ...
        switch (rule.entityLevel) {
            case 'campaign':
                const campaigns = await Campaign_1.default.find(query).select('campaignId name accountId').lean();
                ...
                break;
            case 'adset':
                const adsets = await AdSet_1.default.find(query).select('adsetId name accountId').lean();
                ...
                break;
            case 'ad':
                const ads = await Ad_1.default.find(query).select('adId name accountId').lean();
                entities = ads.map(a => ({
                    id: a.adId,
                    name: a.name,
                    accountId: a.accountId
     ...
     * 执行具体动作
     */
    async executeAction(rule, entity, metrics) {
        const result = {
            entityId: entity.id,
        ...
        try {
            ...
            switch (rule.action.type) {
                case 'auto_pause':
                    ...
                    result.success = true;
                    logger_1.default.info(`[RuleService] Paused ${rule.entityLevel} ${entity.name} (ROAS: ${metrics.roas.toFixed(2)}, Spend: $${metrics.spend.toFixed(2)})`);
                    break;
                case 'auto_enable':
                ...
                case 'budget_up':
                case 'budget_down':
                    const budgetResult = await this.adjustBudget(rule.entityLevel, entity.id, token, rule.action, metrics);
                    ...
                    if (result.success) {
                        logger_1.default.info(`[RuleService] Budget ${rule.action.type === 'budget_up' ? 'increased' : 'decreased'} for ${entity.name}: $${budgetResult.oldBudget} -> $${budgetResult.newBudget} (ROAS: ${metrics.roas.toFixed(2)})`);
                    }
                    break;
                case 'alert':
                    // 发送 Webhook 通知
                    ...
                    result.success = true;
                    logger_1.default.info(`[RuleService] Alert sent for ${entity.name} - ROAS: ${metrics.roas.toFixed(2)}, Spend: $${metrics.spend.toFixed(2)}`);
                    break;
                default:
            ...
            logger_1.default.error(`[RuleService] Action failed for ${entity.name}: ${error.message}`);
        }
        return result;
    }
    /**
     * 暂停实体
     */
    async pauseEntity(level, entityId, token) {
        switch (level) {
            case 'campaign':
                await (0, bulkCreate_api_1.updateCampaign)({ token, campaignId: entityId, status: 'PAUSED' });
                await Campaign_1.default.updateOne({ campaignId: entityId }, { status: 'PAUSED' });
                break;
            case 'adset':
                await (0, bulkCreate_api_1.updateAdSet)({ token, adsetId: entityId, status: 'PAUSED' });
                await AdSet_1.default.updateOne({ adsetId: entityId }, { status: 'PAUSED' });
                break;
            case 'ad':
                await (0, bulkCreate_api_1.updateAd)({ token, adId: entityId, status: 'PAUSED' });
                await Ad_1.default.updateOne({ adId: entityId }, { status: 'PAUSED' });
                break;
        }
    }
    /**
     * 启用实体
     */
    async enableEntity(level, entityId, token) {
        switch (level) {
            case 'campaign':
                await (0, bulkCreate_api_1.updateCampaign)({ token, campaignId: entityId, status: 'ACTIVE' });
                await Campaign_1.default.updateOne({ campaignId: entityId }, { status: 'ACTIVE' });
                break;
            case 'adset':
                await (0, bulkCreate_api_1.updateAdSet)({ token, adsetId: entityId, status: 'ACTIVE' });
                await AdSet_1.default.updateOne({ adsetId: entityId }, { status: 'ACTIVE' });
                break;
            case 'ad':
                await (0, bulkCreate_api_1.updateAd)({ token, adId: entityId, status: 'ACTIVE' });
                await Ad_1.default.updateOne({ adId: entityId }, { status: 'ACTIVE' });
                break;
        }
    }
    /**
     * 调整预算
     */
    async adjustBudget(level, entityId, token, action, metrics) {
        try {
            // 获取当前预算
            let currentBudget = 0;
            if (level === 'campaign') {
                const campaign = await Campaign_1.default.findOne({ campaignId: entityId });
                currentBudget = campaign?.raw?.daily_budget / 100 || 0;
            }
            else if (level === 'adset') {
                const adset = await AdSet_1.default.findOne({ adsetId: entityId });
                currentBudget = adset?.budget || adset?.raw?.daily_budget / 100 || 0;
            }
            else {
            ...
            // 更新预算
            if (level === 'campaign') {
                await (0, bulkCreate_api_1.updateCampaign)({ token, campaignId: entityId, dailyBudget: newBudget });
            }
            else if (level === 'adset') {
                await (0, bulkCreate_api_1.updateAdSet)({ token, adsetId: entityId, dailyBudget: newBudget });
                await AdSet_1.default.updateOne({ adsetId: entityId }, { budget: newBudget });
            }
     ...
     * 发送预警通知
     */
    async sendAlert(rule, entity, metrics) {
        const message = {
            rule: rule.name,
            entity: entity.name,
            entityId: entity.id,
        ...
        if (rule.action.notifyWebhook) {
            try {
                ...
                logger_1.default.info(`[RuleService] Webhook sent to ${rule.action.notifyWebhook}`);
            }
            catch (error) {
                logger_1.default.error(`[RuleService] Webhook failed: ${error.message}`);
            }
        }
        // TODO: 邮件通知
        if (rule.action.notifyEmail) {
            logger_1.default.info(`[RuleService] Email notification to ${rule.action.notifyEmail} (not implemented)`);
        }
    }
    /**
     * 执行所有激活的规则
     */
    async executeAllActiveRules() {
        const activeRules = await AutoRule_1.AutoRule.find({ status: 'active' });
        logger_1.default.info(`[RuleService] Executing ${activeRules.length} active rules...`);
        for (const rule of activeRules) {
            try {
                await this.executeRule(rule._id.toString());
            }
            catch (error) {
                logger_1.default.error(`[RuleService] Failed to execute rule ${rule.name}: ${error.message}`);
            }
        }
    }
    /**
