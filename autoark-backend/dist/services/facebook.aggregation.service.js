...
exports.aggregateMetricsByLevel = void 0;
 ...
 */
const aggregateMetricsByLevel = async (date) => {
    const targetDate = date || (0, dayjs_1.default)().format('YYYY-MM-DD');
    logger_1.default.info(`[Aggregation] Starting metrics aggregation for date: ${targetDate}`);
    try {
        ...
        logger_1.default.info(`[Aggregation] Metrics aggregation completed for date: ${targetDate}`);
    }
    catch (error) {
        logger_1.default.error(`[Aggregation] Failed to aggregate metrics:`, error);
        throw error;
    }
};
exports.aggregateMetricsByLevel = aggregateMetricsByLevel;
/**
 * 聚合 Ad 级别数据到 AdSet 级别
 */
const aggregateAdToAdSet = async (date) => {
    const pipeline = [
        {
            $match: {
                date,
    ...
    const aggregatedData = await MetricsDaily_1.default.aggregate(pipeline).allowDiskUse(true);
    // 保存聚合后的 AdSet 级别数据
    for (const item of aggregatedData) {
        ...
        }, { $set: item }, { upsert: true, new: true });
    }
    logger_1.default.info(`[Aggregation] Aggregated ${aggregatedData.length} adset-level metrics`);
};
/**
 * 聚合 AdSet 级别数据到 Campaign 级别
 */
const aggregateAdSetToCampaign = async (date) => {
    const pipeline = [
        {
            $match: {
                date,
    ...
    logger_1.default.info(`[Aggregation] Aggregated ${aggregatedData.length} campaign-level metrics`);
};
/**
 * 聚合 Campaign 级别数据到 Account 级别
 */
const aggregateCampaignToAccount = async (date) => {
    const pipeline = [
        {
            $match: {
                date,
    ...
    logger_1.default.info(`[Aggregation] Aggregated ${aggregatedData.length} account-level metrics`);
};
