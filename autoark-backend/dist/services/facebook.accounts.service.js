        ...
        for (const tokenDoc of tokens) {
            try {
                ...
                for (const acc of accounts) {
                    const accountData = {
                        channel: 'facebook',
                        accountId: (0, accountId_1.normalizeForStorage)(acc.id), // 统一格式：数据库存储时去掉前缀
                        name: acc.name,
...
const getAccounts = async (filters = {}, pagination, organizationId) => {
    ...
    if (paginatedAccounts.length > 0 && paginatedAccounts.length <= 3) {
        paginatedAccounts.slice(0, 3).forEach((acc) => {
            logger_1.default.info(`Account ${acc.accountId}: periodSpend=${acc.periodSpend}, totalSpend=${acc.totalSpend}, calculatedBalance=${acc.calculatedBalance}`);
        });
    }
    return {
