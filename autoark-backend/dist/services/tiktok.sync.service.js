...
const syncTiktokAdvertiser = async (advertiserId, token) => {
    ...
    try {
        ...
        logger_1.default.info(`[TikTokSync] Syncing ${campaigns.length} campaigns for ${advertiserId}`);
        for (const c of campaigns) {
            await writeToMongo(models_1.Campaign, { campaignId: c.campaign_id }, {
        ...
        logger_1.default.error(`[TikTokSync] Failed to sync campaigns for ${advertiserId}`, err);
    }
    ...
    try {
        ...
        logger_1.default.info(`[TikTokSync] Syncing ${adgroups.length} adgroups for ${advertiserId}`);
        for (const ag of adgroups) {
            await writeToMongo(models_1.AdSet, { adsetId: ag.adgroup_id }, {
        ...
        logger_1.default.error(`[TikTokSync] Failed to sync adgroups for ${advertiserId}`, err);
    }
    // 3. 同步 Ads
    try {
        const adsData = await tiktokApi.fetchTiktokAds(advertiserId, token);
        const ads = adsData.list || [];
        logger_1.default.info(`[TikTokSync] Syncing ${ads.length} ads for ${advertiserId}`);
        for (const a of ads) {
            await writeToMongo(models_1.Ad, { adId: a.ad_id }, {
                adId: a.ad_id,
        ...
        logger_1.default.error(`[TikTokSync] Failed to sync ads for ${advertiserId}`, err);
    }
    // 4. 同步报表 (Hourly)
    try {
