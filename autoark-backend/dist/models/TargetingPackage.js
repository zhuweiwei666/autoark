 ...
 */
const targetingPackageSchema = new mongoose_1.default.Schema({
...
targetingPackageSchema.methods.toFacebookTargeting = function () {
    ...
    if (this.geoLocations) {
        ...
        if (this.geoLocations.regions?.length) {
            targeting.geo_locations.regions = this.geoLocations.regions.map((r) => ({ key: r.key }));
        }
        if (this.geoLocations.cities?.length) {
            targeting.geo_locations.cities = this.geoLocations.cities.map((c) => ({
                key: c.key,
                radius: c.radius,
    ...
    if (this.demographics) {
        if (this.demographics.ageMin)
            targeting.age_min = this.demographics.ageMin;
        if (this.demographics.ageMax)
            targeting.age_max = this.demographics.ageMax;
        if (this.demographics.genders?.length)
            targeting.genders = this.demographics.genders;
    }
    // 兴趣和行为 (flexible_spec)
    const flexibleSpec = [];
    if (this.interests?.length || this.behaviors?.length) {
        ...
        if (this.interests?.length) {
            spec.interests = this.interests.map((i) => ({ id: i.id, name: i.name }));
        }
        if (this.behaviors?.length) {
            spec.behaviors = this.behaviors.map((b) => ({ id: b.id, name: b.name }));
        }
    ...
    if (this.customAudiences?.length) {
        targeting.custom_audiences = this.customAudiences.map((ca) => ({ id: ca.id }));
    }
    // 排除
    if (this.exclusions) {
        const exclusions = {};
        if (this.exclusions.interests?.length) {
            exclusions.interests = this.exclusions.interests.map((i) => ({ id: i.id }));
        }
        if (this.exclusions.behaviors?.length) {
            exclusions.behaviors = this.exclusions.behaviors.map((b) => ({ id: b.id }));
        }
        if (this.exclusions.customAudiences?.length) {
            targeting.excluded_custom_audiences = this.exclusions.customAudiences.map((id) => ({ id }));
        }
...
exports.default = mongoose_1.default.model('TargetingPackage', targetingPackageSchema);
