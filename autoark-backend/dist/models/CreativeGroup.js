...
const creativeGroupSchema = new mongoose_1.default.Schema({
    ...
    // 关联的文案包（可选）
    copywritingPackageId: { type: mongoose_1.default.Schema.Types.ObjectId, ref: 'CopywritingPackage' },
    // 元数据
...
creativeGroupSchema.pre('save', function () {
    if (this.materials) {
        this.materialStats = {
            ...
            videoCount: this.materials.filter((m) => m.type === 'video').length,
            uploadedCount: this.materials.filter((m) => m.status === 'uploaded').length,
        };
    }
});
// 获取已上传的素材
creativeGroupSchema.methods.getUploadedMaterials = function () {
    return this.materials.filter((m) => m.status === 'uploaded');
};
// 获取第一个可用素材（用于单图/视频广告）
...
// 转换为 Facebook 轮播广告格式
creativeGroupSchema.methods.toFacebookCarousel = function (copywriting) {
    const uploadedMaterials = this.getUploadedMaterials();
    if (uploadedMaterials.length === 0)
        return null;
    const carouselCards = uploadedMaterials.slice(0, 10).map((material, index) => {
        const card = {
...
creativeGroupSchema.methods.isReady = function () {
    return this.materials.length > 0 &&
        this.materials.every((m) => m.status === 'uploaded');
};
exports.default = mongoose_1.default.model('CreativeGroup', creativeGroupSchema);
