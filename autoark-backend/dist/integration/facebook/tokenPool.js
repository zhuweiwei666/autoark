...
class TokenPool {
    ...
    async initialize() {
        try {
            const tokenDocs = await FbToken_1.default.find({ status: 'active' })
            ...
            logger_1.default.info(`[TokenPool] Initialized with ${this.tokens.length} tokens`);
        }
        catch (error) {
            logger_1.default.error('[TokenPool] Failed to initialize:', error);
            throw error;
        }
    }
    /**
    ...
    getNextToken() {
        if (this.tokens.length === 0) {
            logger_1.default.warn('[TokenPool] No tokens available');
            return null;
        }
        // 过滤出可用的 tokens（非限流、非失败、非禁用）
        const availableTokens = this.tokens.filter((t) => t.status === 'active' &&
            (!t.rateLimitUntil || t.rateLimitUntil < new Date()));
        if (availableTokens.length === 0) {
            logger_1.default.warn('[TokenPool] No available tokens, all are rate limited or failed');
    ...
    markTokenFailure(token, error) {
        ...
        if (errorMessage.includes('rate limit') ||
            ...
            logger_1.default.warn(`[TokenPool] Token ${tokenObj.id} rate limited, will retry after ${tokenObj.rateLimitUntil.toISOString()}`);
        }
        else if (tokenObj.failureCount >= 3) {
            // 连续失败 3 次：标记为失败
            tokenObj.status = 'failed';
            logger_1.default.error(`[TokenPool] Token ${tokenObj.id} marked as failed after ${tokenObj.failureCount} failures`);
        }
    }
    /**
     * 标记 Token 成功（重置失败计数）
    ...
    markTokenSuccess(token) {
        ...
        if (tokenObj.failureCount > 0) {
            ...
            logger_1.default.info(`[TokenPool] Token ${tokenObj.id} recovered`);
        }
        // 清除限流标记（如果已过期）
    ...
    switchToToken(tokenId) {
        ...
        tokenObj.priority = 0;
        logger_1.default.info(`[TokenPool] Switched to token ${tokenId}`);
    }
}
// 单例模式
exports.tokenPool = new TokenPool();
