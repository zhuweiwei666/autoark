...
export const runRulesForAdSet = async (adsetId: string, date?: string) => {
  ...
  for (const metrics of metricsList) {
    ...
    if (triggeredRules.length > 0) {
      logger.info(
        `Rules triggered for Ad ${metrics.adId}: ${triggeredRules.map((r) => r.name).join(', ')}`,
      )
...
export const runRulesDaily = async () => {
  ...
  for (const metric of metrics) {
    ...
    if (triggeredRules.length > 0) {
      totalTriggered++
      logger.info(
        `[RuleEngine] Triggered ${triggeredRules.length} rules for Ad ${metric.adId}`,
      )
  ...
  logger.info(
    `Daily Rule Engine Execution Completed. Triggered rules on ${totalTriggered} records.`,
  )
}
