...
const fetchFacebookDaily = async () => {
  ...
  try {
    ...
    const accounts = await Account.find({ channel: 'facebook', status: 'active' });
    ...
    for (const account of accounts) {
      try {
        logger.info(`Fetching insights for account: ${account.name} (${account.accountId})`);
        // Fetches yesterday's data by default
        await facebookService.getInsightsDaily(account.accountId);
        logger.info(`Successfully fetched insights for account: ${account.accountId}`);
      } catch (error) {
        logger.error(`Failed to fetch insights for account ${account.accountId}`, error);
