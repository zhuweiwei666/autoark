...
export const runAiOptimizerForAdSet = async (adsetId: string) => {
  ...
  if (metrics.length === 0) {
    logger.info(`[AI Optimizer] No metrics found for AdSet ${adsetId}, skipping.`);
    return null;
  }
  ...
  logger.info(`[AI Optimizer] Finished for AdSet ${adsetId}. Actions proposed: ${recommendedActions.length}`);
...
export const runAiOptimizerDaily = async () => {
  ...
  const activeAdSets = await MetricsDaily.distinct('adsetId', { date: yesterday });

  logger.info(`[AI Optimizer] Found ${activeAdSets.length} active AdSets to analyze.`);
  ...
  logger.info(`Daily AI Optimizer Execution Completed. Processed ${processedCount} AdSets.`);
};
