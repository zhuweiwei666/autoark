import logger from '../utils/logger'
import { ANALYZE_ADSET_PROMPT } from './promptTemplates'

interface AIAnalysisResult {
  analysis: string
  reasoning: string
  recommendations: Array<{
    action: string
    params?: any
    confidence: number
  }>
}

// Mock LLM Call
const callLLM = async (prompt: string): Promise<string> => {
  // TODO: Integrate with OpenAI / Groq / Anthropic API
  logger.info('[AI] Mocking LLM call...')

  // Return a static mock response for now
  const mockResponse: AIAnalysisResult = {
    analysis:
      'CPI is stable but CTR is slowly declining. ROI D0 is healthy at 25%.',
    reasoning:
      'The ad set is profitable, but creative fatigue might be setting in due to lower CTR. Increasing budget slightly to capitalize on current ROI is safe.',
    recommendations: [
      {
        action: 'INCREASE_BUDGET',
        params: { amount: 0.1 },
        confidence: 0.85,
      },
      {
        action: 'CHANGE_CREATIVE',
        params: { tags: ['ugc', 'gameplay_v2'] },
        confidence: 0.6,
      },
    ],
  }

  return JSON.stringify(mockResponse)
}

export const analyzeMetrics = async (
  metrics: any[],
): Promise<AIAnalysisResult> => {
  try {
    const metricsString = JSON.stringify(metrics, null, 2)
    const prompt = ANALYZE_ADSET_PROMPT.replace('{{metrics}}', metricsString)

    const rawResponse = await callLLM(prompt)

    // Parse JSON from LLM response (handling potential markdown code blocks if real LLM)
    const jsonStr = rawResponse
      .replace(/```json/g, '')
      .replace(/```/g, '')
      .trim()
    const result: AIAnalysisResult = JSON.parse(jsonStr)

    return result
  } catch (error) {
    logger.error('[AI Analyzer] Failed to analyze metrics', error)
    return {
      analysis: 'Analysis failed due to internal error.',
      reasoning: 'Error in LLM processing.',
      recommendations: [],
    }
  }
}
