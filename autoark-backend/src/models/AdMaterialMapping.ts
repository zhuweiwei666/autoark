...
const adMaterialMappingSchema = new mongoose.Schema(
  {
    // ========== 核心映射 ==========
    ...
    taskId: { type: mongoose.Schema.Types.ObjectId, ref: 'AdTask' }, // 关联的任务
 ...
 * 根据 adId 获取素材 ID
 */
adMaterialMappingSchema.statics.getMaterialId = async function(adId: string): Promise<string | null> {
 ...
 */
adMaterialMappingSchema.statics.getMaterialIds = async function(adIds: string[]): Promise<Map<string, string>> {
 ...
 * 获取素材的所有广告
 */
adMaterialMappingSchema.statics.getAdsByMaterial = async function(materialId: string) {
 ...
 * 统计素材使用情况
 */
adMaterialMappingSchema.statics.getMaterialUsageStats = async function(materialId: string) {
