...
copywritingPackageSchema.methods.toFacebookAdCreative = function(materialUrl?: string, materialType?: string) {
  const creative: any = {
    object_story_spec: {
      link_data: {
  ...
  if (this.urlParameters) {
    const params: string[] = []
    if (this.urlParameters.utmSource) params.push(`utm_source=${this.urlParameters.utmSource}`)
    if (this.urlParameters.utmMedium) params.push(`utm_medium=${this.urlParameters.utmMedium}`)
    if (this.urlParameters.utmCampaign) params.push(`utm_campaign=${this.urlParameters.utmCampaign}`)
    if (this.urlParameters.utmContent) params.push(`utm_content=${this.urlParameters.utmContent}`)
    if (this.urlParameters.customParams) {
      this.urlParameters.customParams.forEach((value: string, key: string) => {
...
copywritingPackageSchema.methods.getFullUrl = function() {
  ...
  const params: string[] = []
  if (this.urlParameters) {
    if (this.urlParameters.utmSource) params.push(`utm_source=${encodeURIComponent(this.urlParameters.utmSource)}`)
    if (this.urlParameters.utmMedium) params.push(`utm_medium=${encodeURIComponent(this.urlParameters.utmMedium)}`)
    if (this.urlParameters.utmCampaign) params.push(`utm_campaign=${encodeURIComponent(this.urlParameters.utmCampaign)}`)
    if (this.urlParameters.utmContent) params.push(`utm_content=${encodeURIComponent(this.urlParameters.utmContent)}`)
  }
