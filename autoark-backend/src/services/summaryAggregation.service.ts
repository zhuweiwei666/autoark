 ...
 */
function extractPurchaseValue(actionValues: any[]): number {
...
// ==================== 仪表盘汇总 ====================
export async function refreshDashboardSummary(date: string): Promise<void> {
  const startTime = Date.now()
  logger.info(`[SummaryAggregation] Refreshing dashboard summary for ${date}`)
  
  try {
    await SummaryMeta.updateOne(
    ...
    if (result.length === 0) {
      logger.info(`[SummaryAggregation] No data for dashboard on ${date}`)
      return
    }
    ...
    await SummaryMeta.updateOne(
      { summaryType: 'dashboard' },
      { $set: { status: 'idle', lastFullRefresh: new Date(), refreshDurationMs: duration } }
    )
    
    logger.info(`[SummaryAggregation] Dashboard summary for ${date} refreshed in ${duration}ms`)
  } catch (error: any) {
    logger.error(`[SummaryAggregation] Dashboard summary refresh failed:`, error)
    await SummaryMeta.updateOne(
      { summaryType: 'dashboard' },
...
// ==================== 账户汇总 ====================
export async function refreshAccountSummary(date: string): Promise<void> {
  const startTime = Date.now()
  logger.info(`[SummaryAggregation] Refreshing account summary for ${date}`)
  ...
  try {
    ...
    const bulkOps = result.map((agg: any) => {
      let revenue = 0
      let purchases = 0
      for (const raw of agg.rawDataList || []) {
        if (raw?.action_values) {
    ...
    if (bulkOps.length > 0) {
      await AccountSummary.bulkWrite(bulkOps)
    }
    
    const duration = Date.now() - startTime
    await SummaryMeta.updateOne(
      { summaryType: 'account' },
      { $set: { status: 'idle', lastFullRefresh: new Date(), refreshDurationMs: duration, recordCount: bulkOps.length } }
    )
    
    logger.info(`[SummaryAggregation] Account summary for ${date} refreshed: ${bulkOps.length} accounts in ${duration}ms`)
  } catch (error: any) {
    logger.error(`[SummaryAggregation] Account summary refresh failed:`, error)
    await SummaryMeta.updateOne(
      { summaryType: 'account' },
      { $set: { status: 'error', lastError: error.message } }
    )
...
// ==================== 国家汇总 ====================
export async function refreshCountrySummary(date: string): Promise<void> {
  const startTime = Date.now()
  logger.info(`[SummaryAggregation] Refreshing country summary for ${date}`)
  ...
  try {
    ...
    logger.info(`[SummaryAggregation] Country summary for ${date} refreshed: ${bulkOps.length} countries in ${duration}ms`)
  } catch (error: any) {
    logger.error(`[SummaryAggregation] Country summary refresh failed:`, error)
    await SummaryMeta.updateOne(
      { summaryType: 'country' },
      { $set: { status: 'error', lastError: error.message } }
    )
...
// ==================== 广告系列汇总 ====================
export async function refreshCampaignSummary(date: string): Promise<void> {
  const startTime = Date.now()
  logger.info(`[SummaryAggregation] Refreshing campaign summary for ${date}`)
  ...
  try {
    ...
    if (bulkOps.length > 0) {
      await CampaignSummary.bulkWrite(bulkOps)
    }
    ...
    logger.info(`[SummaryAggregation] Campaign summary for ${date} refreshed: ${bulkOps.length} campaigns in ${duration}ms`)
  } catch (error: any) {
    logger.error(`[SummaryAggregation] Campaign summary refresh failed:`, error)
    await SummaryMeta.updateOne(
      { summaryType: 'campaign' },
      { $set: { status: 'error', lastError: error.message } }
    )
...
// ==================== 素材汇总 ====================
export async function refreshMaterialSummary(date: string): Promise<void> {
  const startTime = Date.now()
  logger.info(`[SummaryAggregation] Refreshing material summary for ${date}`)
  ...
  try {
    ...
    // 获取 Ad -> Creative 映射
    const ads = await Ad.find({}).select('adId creativeId imageHash videoId thumbnailUrl').lean()
    ...
    // 按 Ad 聚合，然后按素材合并
    const adMetrics = await MetricsDaily.aggregate([
    ...
    for (const adMetric of adMetrics) {
      ...
      if (adMetric.campaignId) agg.campaignIds.add(adMetric.campaignId)
    ...
    for (const [materialKey, agg] of materialAgg) {
      ...
      // 计算质量分
      let qualityScore = 50
      if (metrics.roas >= 3) qualityScore += 30
      else if (metrics.roas >= 2) qualityScore += 25
      else if (metrics.roas >= 1.5) qualityScore += 20
      else if (metrics.roas >= 1) qualityScore += 10
      else if (metrics.roas < 0.5) qualityScore -= 10
      
      if (metrics.ctr >= 2) qualityScore += 10
      else if (metrics.ctr >= 1) qualityScore += 5
      else if (metrics.ctr < 0.5) qualityScore -= 5
    ...
    if (bulkOps.length > 0) {
      await MaterialSummary.bulkWrite(bulkOps)
    }
    ...
    logger.info(`[SummaryAggregation] Material summary for ${date} refreshed: ${bulkOps.length} materials in ${duration}ms`)
  } catch (error: any) {
    logger.error(`[SummaryAggregation] Material summary refresh failed:`, error)
    await SummaryMeta.updateOne(
      { summaryType: 'material' },
      { $set: { status: 'error', lastError: error.message } }
    )
...
// ==================== 刷新所有汇总 ====================
export async function refreshAllSummaries(date?: string): Promise<{
  dashboard: boolean
  account: boolean
  country: boolean
  campaign: boolean
  ...
  logger.info(`[SummaryAggregation] Refreshing all summaries for ${targetDate}`)
  ...
  try {
    ...
    logger.error('[SummaryAggregation] Dashboard refresh failed', e)
  }
  ...
  try {
    ...
    logger.error('[SummaryAggregation] Account refresh failed', e)
  }
  ...
  try {
    ...
    logger.error('[SummaryAggregation] Country refresh failed', e)
  }
  ...
  try {
    ...
    logger.error('[SummaryAggregation] Campaign refresh failed', e)
  }
  ...
  try {
    ...
    logger.error('[SummaryAggregation] Material refresh failed', e)
  }
  
  results.duration = Date.now() - startTime
  logger.info(`[SummaryAggregation] All summaries refreshed in ${results.duration}ms`, results)
  
  return results
}

// ==================== 获取汇总状态 ====================
export async function getSummaryStatus(): Promise<any> {
  const metas = await SummaryMeta.find({}).lean()
  return metas.reduce((acc: any, m: any) => {
    acc[m.summaryType] = {
      status: m.status,
