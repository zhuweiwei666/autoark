...
function parseIpv6(input: string): bigint | null {
  ...
  if (!address || address.includes('%') || address.split('::').length > 2) return null
...
export const isPublicIpAddress = (input: string): boolean => {
  ...
  if (family === 4) {
    ...
    return !BLOCKED_IPV4_CIDRS.some(([network, prefix]) => ipv4InCidr(numeric, network, prefix))
  }
  ...
  return !BLOCKED_GLOBAL_IPV6_CIDRS.some(([network, prefix]) => ipv6InCidr(numeric, network, prefix))
}
...
const redirectLimit = (value: number | undefined): number => {
  if (typeof value !== 'number' || !Number.isFinite(value)) return DEFAULT_MAX_REDIRECTS
  return Math.min(MAX_REDIRECTS, Math.max(0, Math.floor(value)))
}

const normalizeOptions = (options: RemoteMediaDownloadOptions) => ({
  maxBytes: positiveInteger(options.maxBytes, DEFAULT_MAX_BYTES, ABSOLUTE_MAX_BYTES),
...
const defaultResolve = async (hostname: string): Promise<ReadonlyArray<RemoteMediaAddress>> => {
  const addresses = await dns.promises.lookup(hostname, { all: true, verbatim: true })
  return addresses.map(({ address, family }) => ({ address, family: family as 4 | 6 }))
}

const abortError = (
  category: 'total_timeout' | 'cancelled',
  host: string,
...
const validateAndPinAddress = async (
  ...
  if (literalFamily) {
    ...
    try {
      resolved = await raceWithAbort(Promise.resolve(resolve(hostname)), signal, getAbortError)
    } catch (error) {
  ...
  const validated = resolved.map((entry) => {
    const family = typeof entry?.address === 'string' ? net.isIP(stripIpv6Brackets(entry.address)) : 0
...
const normalizeMimeType = (header: string | undefined): string | null => {
  const mimeType = String(header || '').split(';', 1)[0].trim().toLowerCase()
...
export const createSafeMediaFilename = (url: URL, mimeType: string): string => {
  ...
  if (!extension) throw new RemoteMediaDownloadError('mime_type', sanitizeErrorHost(url.hostname))
  ...
  const withoutControls = decodedBasename
    .normalize('NFC')
    .replace(/[\u0000-\u001f\u007f-\u009f]/g, '')
    .replace(/[\u200b-\u200f\u202a-\u202e\u2060-\u206f\ufeff]/g, '')
  const finalDot = withoutControls.lastIndexOf('.')
  const withoutExtension = finalDot > 0 ? withoutControls.slice(0, finalDot) : withoutControls
  const safeStem = withoutExtension
    ...
    .replace(/_+/g, '_')
    .replace(/^[_-]+|[_-]+$/g, '')
  const stem = trimToUtf8Bytes(safeStem || 'media', MAX_FILENAME_BYTES - Buffer.byteLength(extension))
  return `${stem || 'media'}${extension}`
}

const headerValue = (response: IncomingMessage, name: string): string | undefined => {
...
const requestPinnedMedia = (
  ...
  const finishReject = (error: RemoteMediaDownloadError, destroyResponse = true) => {
    if (settled) return
    settled = true
    cleanup()
  ...
  const lookup: NonNullable<https.RequestOptions['lookup']> = ((
    ...
    if (requestedHostname.replace(/\.$/, '').toLowerCase() !== hostname) {
      callback(Object.assign(new Error('pinned hostname mismatch'), { code: 'EPERM' }), '', 0)
      return
    }
  ...
  const requestOptions: RemoteMediaRequestOptions = {
    ...
    headers: {
      accept: 'image/avif,image/webp,image/png,image/jpeg,image/gif,video/*;q=0.9',
      connection: 'close',
  ...
  try {
    req = request(requestOptions, (incoming) => {
      ...
      if ([301, 302, 303, 307, 308].includes(statusCode)) {
        ...
        incoming.destroy()
        if (!location) {
          finishReject(new RemoteMediaDownloadError('redirect_location', hostname), false)
          return
        }
      ...
      const mimeType = normalizeMimeType(headerValue(incoming, 'content-type'))
      if (!mimeType) {
      ...
      if (contentLengthHeader !== undefined) {
        if (!/^\d+$/.test(contentLengthHeader)) {
          finishReject(new RemoteMediaDownloadError('invalid_response', hostname))
          return
        }
      ...
      incoming.once('end', () => {
        ...
        completed = true
        if (received === 0) {
          finishReject(new RemoteMediaDownloadError('empty_body', hostname), false)
          return
        }
        finishResolve({ kind: 'success', buffer: Buffer.concat(chunks, received), mimeType })
      })
      incoming.once('aborted', () => {
        if (!settled) finishReject(new RemoteMediaDownloadError('network', hostname))
      })
      incoming.once('error', () => {
        if (!settled) finishReject(new RemoteMediaDownloadError('network', hostname))
      })
      incoming.once('close', () => {
        if (!settled && !completed) finishReject(new RemoteMediaDownloadError('network', hostname), false)
      })
    })
  } catch {
  ...
  req.once('socket', (socket) => {
    if ((socket as any).encrypted && !(socket as any).secureConnecting) {
      if (connectTimer) clearTimeout(connectTimer)
      startResponseTimer()
      return
    }
