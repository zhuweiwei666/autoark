...
const authorize = (path: string) => request(createApp())
  .get(path)
...
describe('AI ads read integration', () => {
  beforeEach(() => {
    ...
    ;(FbToken.find as jest.Mock).mockReturnValue(chainResult([
      { _id: 'token-id-1', token: 'meta-token-must-not-leak' },
    ]))
    ;(Account.find as jest.Mock).mockReturnValue(chainResult([
  ...
  it('rejects mixed-currency totals unless the caller selects one currency', async () => {
    ...
    const mixed = await authorize(
      '/api/integrations/ai-ads?dimension=overview',
    )
  ...
  it('returns country metrics only for the server-configured organization scope', async () => {
    ...
    const match = (AggCountryAccount.aggregate as jest.Mock).mock.calls[0][0][0].$match
    expect(match.accountId.$in).toEqual(expect.arrayContaining([
      '123',
      'act_123',
      '456',
      'act_456',
    ]))
    expect(JSON.stringify(response.body)).not.toContain('meta-token-must-not-leak')
  ...
  it('returns a safe overview with distinct active account and campaign counts', async () => {
    ...
    ;(AggAccount.distinct as jest.Mock).mockResolvedValue(['123', '456'])
    ;(AggCampaign.distinct as jest.Mock).mockResolvedValue(['cmp-1', 'cmp-2', 'cmp-3'])
  ...
  it('returns account and campaign dimensions with bounded pagination', async () => {
    ...
    const campaignMatch = (AggCampaign.aggregate as jest.Mock).mock.calls[0][0][0].$match
    expect(campaignMatch.accountId.$in).toEqual(expect.arrayContaining([
      '123',
      'act_123',
      '456',
      'act_456',
